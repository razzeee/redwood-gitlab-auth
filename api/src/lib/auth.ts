...
import { createUser, userByGitlabToken } from 'src/services/users/users'
...
export const getCurrentUser = async (
  ...
  const user = await axios.get(`${process.env.GITLAB_AUTHORITY}/api/v4/user`, {
    headers: {
  ...
  const userExists = await userByGitlabToken({
    gitlabtoken: user.data.username,
  })

  if (userExists) {
    // If you have roles return them!
    return userExists
  } else {
